          ...
          try {
            ...
            outputFile.write(result).then(function(result) {
              ...
              if (config.postprocess != undefined) {
                thiz.busySignalApi.reportBusyWhile(`Postprocessing result of Fudomo transformation "${config.decomposition}"`, function() {
                  const execPromise = exec(config.postprocess, { cwd: outputFile.getParent().getRealPathSync(), windowsHide: true });

                  execPromise.then((stdout, stderr) => {
                    if (notifyOnSuccess) {
