...
import { parseFudomo, loadModel, transform, generateSkeletonModule, MetamodelInferer, TransformationValidator, DataValidator, FudomoComputeException } from 'fudomo-transform' // TODO use ES6 modules
...
export default {
  ...
  async runTransformationConfigFile(path, notifyOnSuccess = true) {
    ...
    try {
      ...
      if (functionSource == null) throw new Error('Functions file not found.');

      phase = new Phase(`parsing Fudomo decomposition functions from "${funcPath}"`, funcFile.getPath());
      const vm = new VM({
          sandbox: { 'module': {} }
      });
      const funcFileRealPath = await funcFile.getRealPath();
      let functionsModule = vm.run(functionSource, funcFileRealPath);
