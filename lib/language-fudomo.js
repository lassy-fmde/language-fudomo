...
import { parseFudomo, loadModel, transform, TransformationContext, getRunnerClassById, getRunnerClassByFileExtension, generateSkeletonModule, MetamodelInferer, TransformationValidator, DataValidator, FudomoComputeException } from 'fudomo-transform' // TODO use ES6 modules
...
function showFudomoComputeException(error, functionsPath, dataPath, decompPath) {
  const options = {
    dismissable: true,
...
export default {
  ...
  async runTransformationConfigFile(configFilePath, notifyOnSuccess = true) {
    ...
    try {
      ...
      // Hack: the source location is an absolute path by default, we want a relative one
      model.center.sourceLocation = atom.project.relativizePath(dataFilePath)[1];
      ...
      if (config.runnerId !== undefined) {
        ...
        RunnerClass = getRunnerClassByFileExtension(path.extname(config.functions).slice(1));
      }
      ...
      const relDecompPath = atom.project.relativizePath(decompFile.getPath())[1];
      const transformation = parseFudomo(decompSource, relDecompPath);
      if (transformation.hasError) {
        ...
        let result = transform(transformationContext);
      ...
      if (error instanceof FudomoComputeException) {
        showFudomoComputeException(error, funcFile.getPath(), dataFile.getPath(), decompFile.getPath());
        return;
      }
