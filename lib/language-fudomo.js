...
function getTransformationErrorsAsMarkDown(transformation) {
  ...
  for (const error of transformation.errors) {
    errorsMd += ' * ' + `${error.location.position[0][0]}:${error.location.position[0][1]} ${error.severity}: ${error.excerpt}\n`;
  }
  return errorsMd;
}
