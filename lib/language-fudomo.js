        ...
        Promise.all([decompPromise, funcPromise]).then(function(parts) {
          ...
          if (transformation.hasError) {
            ...
            try {
              ...
              baseDir.getFile(outputPath).write(result).then(function(result) {
                showSuccess(`Result of Fudomo transformation "${config.decomposition}" successfully written to "${config.output}".`);
              }, function(error) {
      ...
      file.read(true).then(function(decompSource) {
        ...
        if (transformation.hasError) {
          ...
          destFile.write(skeletonSource).then(function(result) {
            showSuccess(`Fudomo function skeletons for transformation "${file.getBaseName()}" successfully written to "${destFile.getBaseName()}".`);
          }, function(error) {
